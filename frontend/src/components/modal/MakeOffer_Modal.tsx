...
export default function MakeOffer_Modal({
    showModal,
    nft,
    highestBid,
    makeAnOffer
      ...
      <div className="relative my-6 mx-auto max-w-3xl w-3/5 xl:w-2/5 2xl:w-1/5">
        ...
        <div className="border-0 rounded-lg shadow-lg relative flex flex-col bg-white outline-none focus:outline-none">
          ...
          <div className='px-6 font-medium text-sm text-gray-500 '>
              <p> You are about to place an offer for <span className='text-black'> {nft.name} #{nft.tokenId}</span>. 
              This offer can be accepted or not by the nft owner.</p>
          </div>
          ...
          <div className="grid mb-4 mx-4">
              <label className="mb-1 font-bold text-base">Offer Price</label>
              <input
              type="number"
              step="0.000001"
              value={tokenPrice}
          ...
          <div className='flex justify-between mx-5 py-1 text-sm text-gray-500'>
              <p> Nft current price </p>
              <p className="text-black"> {nft?.price} ETH </p>
          </div>
          ...
          <div className='flex justify-between mx-5 mt-2 text-sm text-gray-500'>
              <p> Your offer </p>
              <p className="text-black"> {(Number(tokenPrice)).toFixed(5)} ETH </p>
          </div>

          <div className='flex justify-between mx-5 mb-2 text-sm text-gray-500'>
              <p> Total Price </p>
          ...
          <button onClick={() => { 
            showModal(false)
            makeAnOffer((Number(tokenPrice)))
          }}className="bg-neutral mx-4 mt-3 mb-7 py-2 rounded-lg text-white"> 
              Make an offer of {(Number(tokenPrice))} ETH
          </button>
